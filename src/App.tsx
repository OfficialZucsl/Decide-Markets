...
import { collection, onSnapshot, doc, getDocFromServer, runTransaction, query, orderBy, setDoc } from 'firebase/firestore';
...
// Frontend seed data for rendering when DB is empty
const seedMarkets: Market[] = [
  {
    id: 'm1',
    question: "If BoZ hikes rates 200bps, will the Kwacha stay under 25/$ by June?",
    yesPoints: 7200,
    noPoints: 2800,
    category: "Economy",
  ...
  {
    id: 'm2',
    question: "If FISP maize subsidies end, will inflation drop below 10% by July?",
    yesPoints: 3500,
    noPoints: 6500,
    category: "Finance",
  ...
  {
    id: 'm3',
    question: "If Lusaka privatizes waste, will cholera cases drop 50% next rainy season?",
    yesPoints: 8900,
    noPoints: 1100,
  ...
  {
    id: 'm4',
    question: "If the incumbent is re-elected, will their 'Promise-to-Action' legislative ratio exceed 60% in Year 1?",
    yesPoints: 4100,
    noPoints: 5900,
  ...
  {
    id: 'm5',
    question: "If the government waives import duties on solar panels, will solar adoption in rural areas increase by 30%?",
    yesPoints: 5000,
    noPoints: 5000,
    category: "Services",
  ...
  {
    id: 'm6',
    question: "If the new mining tax regime is implemented, will copper production exceed 1 million tonnes next year?",
    yesPoints: 6000,
    noPoints: 4000,
    category: "Mining",
  ...
  {
    id: 'm7',
    question: "If FISP is reformed to include electronic vouchers, will maize yields increase by 20%?",
    yesPoints: 5500,
    noPoints: 4500,
  ...
  {
    id: 'm8',
    question: "If the national broadband policy is updated, will internet penetration reach 60% by year end?",
    yesPoints: 4000,
    noPoints: 6000,
    category: "ICT",
    institution: "ZICTA",
  ...
  {
    id: 'm9',
    question: "If the new SEZs are operationalized, will manufacturing GDP contribution rise by 2%?",
    yesPoints: 4500,
    noPoints: 5500,
  ...
  {
    id: 'm10',
    question: "If the visa waiver program is expanded, will tourist arrivals increase by 15% this year?",
    yesPoints: 5200,
    noPoints: 4800,
    category: "Tourism",
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        // We will mock points for now, or sync from DB later. For MVP, fetch user profile.
        try {
          const userDocRef = doc(db, 'users', currentUser.uid);
          const userSnap = await getDocFromServer(userDocRef);
          if (!userSnap.exists()) {
            // Bootstrap user document for the first time
  ...
  useEffect(() => {
    ...
    const unsubscribeMarkets = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach(doc => {
        ...
        liveMarkets.push({
          id: doc.id,
          question: data.decision + '? \u2192 ' + data.kpi, // Combined for this UI
          ...
          status: data.status
        } as any); // mapped from schema loosely
      });
      // If DB is empty, use the frontend seed data to avoid empty screens
  ...
  const handleVote = async (side: 'YES' | 'NO', amount: number) => {
    ...
    // Check if the market is from our seed or live. If seed, we can't transact on DB realistically yet.
    // Let's implement an optimistic pseudo-transaction for the MVP.
    const isSeed = markets.some(m => seedMarkets.find(sm => sm.id === m.id));
    
    if (isSeed) {
      setMarkets(prev => prev.map(m => {
        if (m.id === selectedMarket.id) {
          const { newQYes, newQNo } = calculateNewShares(m.yesPoints, m.noPoints, amount, side === 'YES');
          return {
            ...m,
            yesPoints: newQYes,
            noPoints: newQNo,
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
         ...
         if (!marketDoc.exists()) throw new Error("Market does not exist");
         
         const currentYes = marketDoc.data().yesPoints;
         const currentNo = marketDoc.data().noPoints;
         
         // Apply strict LMSR math to calculate new mathematical balances
         const { newQYes, newQNo, sharesBought } = calculateNewShares(currentYes, currentNo, amount, side === 'YES');
         
         transaction.update(marketRef, { yesPoints: newQYes, noPoints: newQNo });
         // We also record sharesBought so the user could potentially "cash out" later
         transaction.set(tradeRef, { ...tradeData, sharesBought });
  ...
  const adminCurateMarkets = async () => {
    if (!user || user.email !== 'chipetatexila@gmail.com') return;
    
    // Simulating the Multi-Agent News Curator fetching today's politics news
    const simulatedLiveNews: Market[] = [
      {
        id: 'pol-1',
        decision: "ECZ implements biometric voter verification nationwide",
        kpi: "Opposition petition rate drops by 40% post-elections",
        question: "If ECZ enforces biometric national verification next month, will post-election petitions drop 40%?",
      ...
      {
        id: 'pol-2',
        decision: "Parliament passes the revised Public Order Act",
        kpi: "Police-sanctioned opposition rallies increase by 25%",
        question: "If the new Public Order Act is enacted, will approved opposition rallies increase by 25% by Q4?",
      ...
      {
        id: 'pol-3',
        decision: "Anti-Corruption Commission declares amnesty for asset recovery",
        kpi: "Over K500 million is recovered in 60 days",
        question: "If ACC announces a 60-day asset recovery amnesty, will over K500 million be returned?",
    ...
    try {
      for (const market of simulatedLiveNews) {
        const marketRef = doc(collection(db, 'markets')); // Generate new ID
        const finalMarket = {
        ...
        await setDoc(marketRef, finalMarket);
      }
      alert("Live politics news successfully curated and pushed to Firestore.");
    } catch (error) {
      console.error(error);
      handleFirestoreError(error, OperationType.WRITE, 'markets');
      alert("Failed to push to Firestore. Ensure you are logged in as admin.");
    }
  };
  ...
  const filteredMarkets = markets.filter(market => {
    ...
    if (categoryFilter) {
      if (categoryFilter === 'Trending' || categoryFilter === 'Breaking') {
        // Just show a few random ones for 'Trending' or 'Breaking' for the mockup
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFD] font-sans text-[#1D1D1F] antialiased">
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6 h-16 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-xl font-bold tracking-tight cursor-pointer" onClick={() => { setCategoryFilter(null); setSectorFilter(null); }}>Decide</span>
          </div>
            ...
            <div className="relative group py-4">
              ...
              <div className="absolute top-full left-0 mt-[-8px] w-48 bg-white/90 backdrop-blur-xl border border-gray-100 rounded-2xl shadow-xl opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300 overflow-hidden">
                <div className="p-2 flex flex-col">
                  ...
                  <button onClick={() => { setCategoryFilter('Candidate'); setSectorFilter(null); }} className="text-left px-4 py-2 hover:bg-blue-50 text-blue-600 rounded-xl transition-colors flex items-center justify-between">
                    <span>Candidates</span>
                    <span className="text-[10px] bg-blue-100 px-2 py-0.5 rounded-full">KPIs</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center space-x-6">
            {!user ? (
              <button 
                onClick={handleLogin}
                className="bg-black text-white px-4 py-1.5 rounded-full text-sm font-semibold hover:bg-gray-800 transition-colors flex items-center space-x-2"
              >
      ...
      <main className="max-w-7xl mx-auto px-6 pt-16 pb-24 flex flex-col md:flex-row gap-12 lg:gap-16">
        ...
        <aside className="w-full md:w-48 shrink-0">
          <div className="sticky top-28 flex flex-col space-y-8">
            <div className="flex flex-col space-y-1">
              <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 px-4">Timeline</h3>
            ...
            <div className="flex flex-col space-y-1">
              <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 px-4">Sectors</h3>
            ...
            {user && user.email === 'chipetatexila@gmail.com' && (
              <div className="pt-8">
                 <button
        ...
        <div className="flex-1 min-w-0">
          ...
          >
            ...
            <span className="text-blue-600">not just Outcomes.</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredMarkets.length > 0 ? filteredMarkets.map((market, idx) => {
            const prob = calculateProbability(market.yesPoints, market.noPoints) * 100;
            return (
              <motion.div
                key={market.id}
                layoutId={market.id}
              ...
              >
                <div className="flex justify-between items-start mb-8">
                  ...
                  <span className="text-xs font-bold text-gray-300 tracking-widest">{market.category}</span>
                </div>
                ...
                <div className="flex-grow mb-8">
                  <h3 className="text-xl font-semibold leading-snug text-gray-900 group-hover:text-blue-600 transition-colors duration-300">
                    {market.question || (market as any).decision} 
                  </h3>
                </div>
                ...
                <div className="mt-auto">
                  ...
                  <div className="mt-4 flex justify-between text-[11px] font-bold text-gray-400 uppercase tracking-widest">
                    ...
                    <span>{ (market.yesPoints + market.noPoints).toLocaleString() } Votes</span>
                  </div>
                </div>
              </motion.div>
            );
            ...
            <div className="col-span-full py-20 text-center text-gray-400 font-medium">
              No active markets found for this filter. AI Agents are currently scraping the latest news...
            </div>
          )}
        </div>
        </div>
      ...
      {/* --- Steve Jobs-esque Voting Overlay --- */}
      <AnimatePresence>
        {selectedMarket && (
          <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center p-4 sm:p-6">
            ...
            >
              <h3 className="text-3xl font-bold mb-10 leading-tight text-gray-900">
                {selectedMarket.question || (selectedMarket as any).decision}
              </h3>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                ...
                >
                  {isVoting ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Yes'}
                </button>
                <button 
                ...
                >
                  {isVoting ? <Loader2 className="w-6 h-6 animate-spin" /> : 'No'}
                </button>
              </div>
              
              <div className="text-center text-sm font-medium italic">
                {user ? (
                   <span className="text-gray-400">Allocating 100 points. Resolution verified by {selectedMarket.institution}.</span>
                ) : (
                   <span className="text-red-500 font-bold">You must log in to allocate points.</span>
                )}
              </div>
            </motion.div>
          </div>
